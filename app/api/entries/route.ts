...
export async function GET(req: Request) {
  try {
    ...
    let query = supabaseAdmin
      .from('contest_entries')
      .select('entry_id,contest_id,work_id,user_id,work_number,entry_type,name,name_kana,status,is_primary_passed,created_at,school_name,grade,guardian_name,guardian_email,guardian_phone,guardian_consent,guardian_consent_at,works(title,category,short_description,detailed_description,work_url,video_type,video_location,thumbnail_url),contests(title,year,status),users(name)')
